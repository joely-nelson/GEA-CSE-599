...
### Modified from example to keep the probabilities during evaluation, useful to compute top10 accuracy
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if args.output_mode == "classification":
            if args.task_name[:3] == "dna" and args.task_name != "dnasplice":
                ...
                probs = softmax(torch.tensor(preds, dtype=torch.float32)).numpy()
            # MOD
            ## If multiclass specified, get all probs not just binary case
            if args.multiclass:
                probs = softmax(torch.tensor(preds, dtype=torch.float32)).numpy()
            ...
            # MOD add option to save probs, labels and preds for the evaluation
            # will be kept in a 3d array (iteration, eval_sample, class_prob)
            if args.save_eval_probs:
                eval_probs_array = os.path.join(eval_output_dir, prefix, "eval_probs_results.npy")
                # keep out label ids just to confirm they are all the same
                eval_label_array = os.path.join(eval_output_dir, prefix, "eval_label_keys.npy")
                # keep preds too
                eval_pred_array = os.path.join(eval_output_dir, prefix, "eval_pred_results.npy")
                if not os.path.exists(eval_probs_array):
                    # initialize the object
                    np.save(eval_probs_array, [probs])
                    np.save(eval_label_array, [out_label_ids])
                    np.save(eval_pred_array, [preds])
                else:
                    # it is not the first evaluation, so load previous ones and append to array
                    prev_prob = np.load(eval_probs_array)
                    np.save(eval_probs_array, np.vstack((prev_prob, [probs])))
                    prev_labl = np.load(eval_label_array)
                    np.save(eval_label_array, np.vstack((prev_labl, [out_label_ids])))
                    prev_pred = np.load(eval_pred_array)
                    np.save(eval_pred_array, np.vstack((prev_pred, [preds])))
        elif args.output_mode == "regression":
            preds = np.squeeze(preds)
        if args.do_ensemble_pred:
            result = compute_metrics(eval_task, preds, out_label_ids, probs[:,1])
        else:
            # note that when args.multiclass is True probs are (nsamples, nclasses)
            # if False, it is (nsamples,)
...
def main():
    ...
    # MOD add option to keep probabilities, labels and preds of evaluation predictions
    parser.add_argument(
        "--save_eval_probs", action="store_true", help="Keep prediction probabilities for every evaluation step in a np array"
    )
    parser.add_argument(
        "--multiclass", action="store_true", help="It is a mutliclass case, considered when computing metrics"
    )
